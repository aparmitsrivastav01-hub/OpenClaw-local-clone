    ...
    return {
      ...
      fetch_url: tool({
        ...
        execute: async ({ url }) => {
          ...
          const blockedPatterns = ['localhost', '127.', '0.0.0.0', '::1', '169.254.', '192.168.', '10.', '172.16.', '172.17.', '172.18.', '172.19.', '172.20.', '172.21.', '172.22.', '172.23.', '172.24.', '172.25.', '172.26.', '172.27.', '172.28.', '172.29.', '172.30.', '172.31.'];
          if (blockedPatterns.some(pattern => hostname.includes(pattern))) {
            throw new Error(`Blocked URL: Access to ${hostname} is not allowed for security reasons`);
          }
          if (!['http:', 'https:'].includes(parsedUrl.protocol)) {
            throw new Error(`Blocked protocol: ${parsedUrl.protocol} is not allowed`);
          }
